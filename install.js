...
if (!ffmpegPath) {
  exitOnError(
    "ffmpeg-static-mirror install failed: No binary found for architecture"
  );
}
...
try {
  if (fs.statSync(ffmpegPath).isFile()) {
    console.info(
      "ffmpeg-static-mirror reports that ffmpeg is installed already."
    );
    process.exit(0);
  }
} catch (err) {
...
function onProgress(deltaBytes, totalBytes) {
  ...
  if (!progressBar) {
    progressBar = new ProgressBar(
      `Downloading ffmpeg ${releaseName} [:bar] :percent :etas `,
      {
        complete: "|",
        incomplete: " ",
        width: 20,
...
const base =
  process.env.THE_BASE_URL ||
  "https://mirror.ghproxy.com/https://github.com/descriptinc/ffmpeg-ffprobe-static/releases/download/";
console.log(`[ffmpeg-static-mirror] Using base url: ${base}`);
